...
export default function BrandsPage() {
  ...
  return (
    <main className="flex-1 w-full bg-white min-h-screen">
      <div className="pt-32 pb-20 px-4 md:px-8 border-b border-blue-100">
            ...
            {INDUSTRIES.map((ind) => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${
                  filter === ind 
                    ? "bg-[#0B2EA8] text-white" 
                    : "glass text-blue-700/70 hover:text-[#0B2EA8] hover:bg-blue-50"
                }`}
              >
                {ind}
              </button>
            ...
            <AnimatePresence>
              {filteredBrands.map((brand) => (
                <motion.div
                  ...
                  className="aspect-video glass-panel border border-blue-100 rounded-3xl flex flex-col items-center justify-center p-6 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(59,130,246,0.12)] transition-all duration-500 group relative overflow-hidden"
                >
                  ...
                  <span className="text-5xl md:text-6xl font-black font-heading tracking-tighter text-gradient transition-all relative z-10">
                    {brand.name}
                  </span>
                  <p className="mt-4 text-[10px] font-bold uppercase tracking-widest text-blue-700/50 group-hover:text-[#0B2EA8] transition-colors relative z-10">{brand.industry}</p>
                </motion.div>
              ))}
            </AnimatePresence>
          </motion.div>
      ...
      <section className="py-32 px-4 md:px-8 bg-white">
        <div className="container mx-auto max-w-4xl text-center">
           <div className="glass p-12 md:p-20 rounded-3xl relative">
              <div className="text-[120px] font-heading leading-none text-blue-100 absolute top-4 left-8">"</div>
              ...
              <div className="flex items-center justify-center gap-4">
                 ...
                 <div className="text-left">
                    ...
                    <p className="text-blue-800/60 text-sm">Fortune 500 Tech Brand</p>
                 </div>
              </div>
           </div>
        </div>
